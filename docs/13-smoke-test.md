...
linode-cli firewalls rules-update \
  --rules.inbound "[{\"ports\": \"$NODE_PORT\", \"protocol\": \"TCP\", \"addresses\": { \"ipv4\": [ \"0.0.0.0/0\"] }, \"action\": \"ACCEPT\", \"label\": \"nginx-remote-access\", \"description\": \"allow remote access to test nginx service\"}]" \
...
```
INSTANCE_ID=$(linode-cli linodes list --label $INSTANCE_NAME --json | jq -r '.[].id')
...
HTTP/1.1 200 OK
Server: nginx/1.17.3
Date: Tue, 27 Jul 2021 13:54:34 GMT
Content-Type: text/html
Content-Length: 612
...
```sh
instance_id=$(linode-cli linodes list --label worker-0 --json \
  | jq -r '.[].id')
