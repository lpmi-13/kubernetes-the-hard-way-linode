...
ExecStart=/usr/local/bin/kube-apiserver \\
  ...
  --etcd-servers=https://${controller_0_private_ip}:2379,https://${controller_1_private_ip}:2379,https://${controller_2_private_ip}:2379 \\
  --event-ttl=1h \\
...
```
controller_0_instance_id=$(linode-cli linodes list --label controller-0 \
  --json | jq -r '.[].id')
